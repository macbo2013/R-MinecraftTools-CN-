...
class PacketSniffer:
                ...
                try:
                    ...
                    for line in lines:
                        if "adapter" in line.lower() and ":" in line:
                            ...
                            interfaces.append({"name": current_iface, "description": current_iface})
            else:
                # Linux/macOS系统
    ...
    def get_protocol_statistics(self, packets):
        """获取协议统计信息"""
        stats = {
            'total': len(packets),
            'tcp': 0,
            ...
            'icmp': 0,
            'other': 0,
            'size_distribution': {'<100': 0, '100-500': 0, '500-1000': 0, '>1000': 0}
        }
        ...
        for packet in packets:
            try:
                ...
                if size < 100:
                    ...
                    stats['size_distribution']['>1000'] += 1
            except:
                pass
...
class MinecraftAttacker(QMainWindow):
    ...
    def initUI(self):
        self.setWindowTitle("run++科技 - v.3.0-")
    ...
    def setup_network_tools_tab(self):
        """添加网络工具标签页"""
        layout = QVBoxLayout(self.network_tools_tab)
        ...
        scan_layout.addWidget(QLabel("目标网络:"), 0, 0)
        self.scan_network = QLineEdit("192.168.1.0/24")
        scan_layout.addWidget(self.scan_network, 0, 1)
        
        scan_layout.addWidget(QLabel("端口范围:"), 1, 0)
        self.scan_ports = QLineEdit("1-1000")
        scan_layout.addWidget(self.scan_ports, 1, 1)
        
        scan_layout.addWidget(QLabel("线程数:"), 2, 0)
        self.scan_threads = QSpinBox()
        self.scan_threads.setRange(1, 100)
        self.scan_threads.setValue(10)
        scan_layout.addWidget(self.scan_threads, 2, 1)
        
        scan_button_layout = QHBoxLayout()
        self.scan_hosts_btn = QPushButton("扫描主机")
        self.scan_hosts_btn.clicked.connect(self.scan_network_hosts)
        scan_button_layout.addWidget(self.scan_hosts_btn)
        
        self.scan_ports_btn = QPushButton("扫描端口")
        self.scan_ports_btn.clicked.connect(self.scan_network_ports)
        scan_button_layout.addWidget(self.scan_ports_btn)
        
        scan_layout.addLayout(scan_button_layout, 3, 0, 1, 2)
        ...
        connection_layout.addWidget(QLabel("测试目标:"), 0, 0)
        self.test_target = QLineEdit("example.com")
        connection_layout.addWidget(self.test_target, 0, 1)
        
        connection_layout.addWidget(QLabel("端口:"), 1, 0)
        self.test_port = QLineEdit("80")
        connection_layout.addWidget(self.test_port, 1, 1)
        
        test_button_layout = QHBoxLayout()
        self.ping_test_btn = QPushButton("Ping测试")
        self.ping_test_btn.clicked.connect(self.test_ping)
        test_button_layout.addWidget(self.ping_test_btn)
        
        self.tcp_test_btn = QPushButton("TCP连接测试")
        self.tcp_test_btn.clicked.connect(self.test_tcp_connection)
        test_button_layout.addWidget(self.tcp_test_btn)
        
        connection_layout.addLayout(test_button_layout, 2, 0, 1, 2)
        
        connection_group.setLayout(connection_layout)
        layout.addWidget(connection_group)
        
        # 协议分析功能
        analysis_group = QGroupBox("协议分析")
        analysis_layout = QVBoxLayout()
        
        self.analyze_btn = QPushButton("分析抓包数据")
        ...
        self.network_info_btn = QPushButton("获取网络信息")
        self.network_info_btn.clicked.connect(self.show_network_info)
        info_layout.addWidget(self.network_info_btn)
        
        self.network_info_text = QTextEdit()
        self.network_info_text.setMaximumHeight(100)
        ...
        log_layout.addWidget(self.network_log)
        log_group.setLayout(log_layout)
        layout.addWidget(log_group)
    
    def scan_network_hosts(self):
        """扫描网络中的活跃主机"""
        network = self.scan_network.text()
        try:
            # 验证网络格式
            ip_network = ipaddress.ip_network(network, strict=False)
            self.network_log.append(f"[+] 开始扫描网络: {network}")
            
            # 在实际应用中，这里应该使用多线程进行ARP扫描或ICMP扫描
            # 这里使用简化的模拟实现
            def scan_thread():
                try:
                    # 模拟扫描过程
                    for i in range(1, 11):
                        time.sleep(0.5)
                        ip = f"{ip_network.network_address + i}"
                        self.network_log.append(f"发现主机: {ip} (模拟)")
                    
                    self.network_log.append("[+] 扫描完成")
                except Exception as e:
                    self.network_log.append(f"[-] 扫描错误: {str(e)}")
            
            threading.Thread(target=scan_thread, daemon=True).start()
            
        except ValueError:
            QMessageBox.warning(self, "错误", "请输入有效的网络地址，如: 192.168.1.0/24")
    
    def scan_network_ports(self):
        """扫描目标主机的开放端口"""
        target = self.scan_network.text().split('/')[0]  # 取IP部分
        ...
        try:
            ...
            self.network_log.append(f"[+] 开始扫描端口: {target}:{start_port}-{end_port}")
            
            # 模拟端口扫描
            def port_scan_thread():
                try:
                    # 模拟发现一些常见端口
                    common_ports = [21, 22, 23, 25, 53, 80, 110, 135, 139, 143, 443, 445, 993, 995, 19132]
                    
                    for port in range(start_port, min(end_port + 1, start_port + 50)):
                        if port in common_ports and random.random() > 0.7:
                            self.network_log.append(f"发现开放端口: {target}:{port} (模拟)")
                        time.sleep(0.01)
                    
                    self.network_log.append("[+] 端口扫描完成")
                except Exception as e:
                    self.network_log.append(f"[-] 端口扫描错误: {str(e)}")
            
            threading.Thread(target=port_scan_thread, daemon=True).start()
            
        except ValueError:
            QMessageBox.warning(self, "错误", "请输入有效的端口范围，如: 1-1000")
    
    def test_ping(self):
        """测试网络连通性"""
        target = self.test_target.text()
        if not target:
            QMessageBox.warning(self, "错误", "请输入测试目标")
            return
        
        self.network_log.append(f"[+] 开始Ping测试: {target}")
        ...
        def ping_thread():
            try:
                ...
                if "回复" in result.stdout or "bytes from" in result.stdout:
                    self.network_log.append(f"[+] Ping成功: {target}")
                else:
                    self.network_log.append(f"[-] Ping失败: {target}")
                    
            except Exception as e:
                self.network_log.append(f"[-] Ping测试错误: {str(e)}")
    ...
    def test_tcp_connection(self):
        """测试TCP连接"""
        target = self.test_target.text()
        port = self.test_port.text()
        
        if not target or not port:
            QMessageBox.warning(self, "错误", "请输入测试目标和端口")
            return
        
        try:
            port = int(port)
            self.network_log.append(f"[+] 测试TCP连接: {target}:{port}")
            ...
            def tcp_test_thread():
                try:
                    ...
                    if result == 0:
                        self.network_log.append(f"[+] TCP连接成功: {target}:{port}")
                    else:
                        self.network_log.append(f"[-] TCP连接失败: {target}:{port} (错误码: {result})")
                        
                except Exception as e:
                    self.network_log.append(f"[-] TCP测试错误: {str(e)}")
            
            threading.Thread(target=tcp_test_thread, daemon=True).start()
    ...
    def analyze_packets(self):
        ...
        if not self.sniffer.packets:
            QMessageBox.warning(self, "警告", "没有可分析的抓包数据")
            return
        
        try:
            stats = self.sniffer.get_protocol_statistics(self.sniffer.packets)
            
            analysis_text = "=== 协议分析结果 ===\n\n"
            analysis_text += f"总数据包数: {stats['total']}\n"
            analysis_text += f"TCP包: {stats['tcp']}\n"
            analysis_text += f"UDP包: {stats['udp']}\n"
            analysis_text += f"ICMP包: {stats['icmp']}\n"
            analysis_text += f"其他协议: {stats['other']}\n\n"
            
            analysis_text += "=== 数据包大小分布 ===\n"
            analysis_text += f"<100字节: {stats['size_distribution']['<100']}\n"
            analysis_text += f"100-500字节: {stats['size_distribution']['100-500']}\n"
            analysis_text += f"500-1000字节: {stats['size_distribution']['500-1000']}\n"
            analysis_text += f">1000字节: {stats['size_distribution']['>1000']}\n"
            
            self.analysis_result.setPlainText(analysis_text)
            self.network_log.append("[+] 协议分析完成")
            
        except Exception as e:
            self.network_log.append(f"[-] 分析错误: {str(e)}")
    ...
    def show_network_info(self):
        ...
        try:
            ...
            try:
                ...
                s.close()
                info_text += f"本地IP: {ip_address}\n"
            except:
                info_text += "本地IP: 无法获取\n"
            
            # 获取网络接口信息
            interfaces = self.sniffer.get_network_interfaces()
            info_text += f"\n网络接口: {len(interfaces)} 个\n"
            
            for i, iface in enumerate(interfaces[:3]):  # 只显示前3个
                if isinstance(iface, dict):
                    info_text += f"{i+1}. {iface.get('name', '未知')} - {iface.get('description', '无描述')}\n"
                else:
                    info_text += f"{i+1}. {iface}\n"
            
            if len(interfaces) > 3:
                info_text += f"... 还有 {len(interfaces) - 3} 个接口未显示\n"
            
            self.network_info_text.setPlainText(info_text)
            self.network_log.append("[+] 网络信息获取完成")
            
        except Exception as e:
            self.network_log.append(f"[-] 获取网络信息错误: {str(e)}")
